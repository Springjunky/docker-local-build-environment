...
doDownload() {
    ...
    if test -e  "$jpi" 
      then
        echo "Plugin $jpi already downloaded "
      else  
        curl --connect-timeout "${CURL_CONNECTION_TIMEOUT:-20}" --retry "${CURL_RETRY:-5}" --retry-delay "${CURL_RETRY_DELAY:-1}" --retry-max-time "${CURL_RETRY_MAX_TIME:-60}" -s -f -L "$url" -o "$jpi"
    fi   
    return $?
}
